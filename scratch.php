...
function execute_xtb_stream_command($sp, $streamSessionId,  $command, $params = [], $args = null)
{
    execute_xtb_command($sp, $command, array_merge(["streamSessionId" => $streamSessionId], $params), $args, true);
}

function execute_xtb_command($sp, string $command, array $params = [], $args = null, bool $stream = false): ?string
{
    $data = array_merge($params, ["command" => $command]);
    if ($args !== null && is_array($args)) {
        $data['arguments'] = $args;
    }

    websocket_write($sp, json_encode($data), true, false);
    if (!$stream) {
        return websocket_read($sp);
    }
...
$login = json_encode(
    [
        ...
        "arguments" => [
            "userId" => "1869888",
            "password" => "NKm?77*!N3Q.aQT",
        ],
        "customTag" => "my_login_command_id"
    ]
);

if ($sp = websocket_open('ws.xtb.com', 443, '', $errstr, 10, true, false, '/real')) {
    $login = json_decode(execute_xtb_command($sp, 'login', [], ["userId" => "1869888", "password" => "NKm?77*!N3Q.aQT"]), true);
    $symbols = json_decode(execute_xtb_command($sp, 'getAllSymbols'), true);

    if ($sps = websocket_open('ws.xtb.com', 443, '', $errstr, 10, true, false, '/realStream')) {
        execute_xtb_stream_command(
            $sps,
            $login['streamSessionId'],
            'getKeepAlive'
        );
        execute_xtb_stream_command(
            $sps,
            $login['streamSessionId'],
            'getTickPrices',
            ['symbol' => 'EURUSD', 'maxLevel'=>0, 'minArrivalTime' =>1]
        );
        execute_xtb_stream_command(
            $sps,
            $login['streamSessionId'],
            'getTickPrices',
            ['symbol' => 'USDJPY', 'maxLevel' =>0, 'minArrivalTime' => 1]
        );
        execute_xtb_stream_command(
            $sps,
            $login['streamSessionId'],
            'getTickPrices',
            ['symbol' => 'USDCHF', 'maxLevel' =>0, 'minArrivalTime' => 1]
        );
    }
    ...
    while (true) {
        $error = "";
        if ($data = websocket_read($sps, $error)) {
            var_dump($data);
